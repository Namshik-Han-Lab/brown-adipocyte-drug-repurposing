...
In this vignette, we analyse three datasets of single-nucleus RNA sequencing of human perivascular adipose tissue (PVAT). There are 12’657, 6’084 and 6’456 cells in three samples respectively that were sequenced on the Illumina NovaSeq 6000.

The raw data can be found on Gene Expression Omnibus [at this link](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE164528).

This script builds on code originally developed by Holly A. R. Giles.
...
# Define labels
y_labels <- c("nFeature_RNA (genes)", "nCount_RNA (molecules)", "percent.mt (%)")

# Create one violin plot per feature
plots <- lapply(seq_along(features), function(i) {
  f <- features[i]
  VlnPlot(combined, features = f, group.by = "sample", pt.size = 0.000000000001) +
    ylab(y_labels[i]) +
...
pdf("results/humanPVATsn/final_figs/complete_PVAT_analysis.pdf", width = 14, height = 16)
print(final_figure)
invisible(dev.off())
...
Idents(subset_obj) <- factor(Idents(subset_obj),
                                        levels = c("Early preadipocytes", 
                                                   "Intermediate preadipocytes", 
                                                   "Transitional adipocytes", 
                                                   "Mature adipocytes"))
...
avg_exp <- AverageExpression(subset_obj, features = genes_to_plot, 
                             return.seurat = TRUE)$RNA
...
# Save to PDF
pdf("results/humanPVATsn/final_figs/amyloid_gene_heatmap.pdf", width = 11, height = 7)
print(heat_map)
# Add legend title manually
grid::grid.text("Average expression", x = 0.915, y = 0.98, rot = 0, gp = grid::gpar(fontsize = 12))
invisible(dev.off())
...
print(neuro_heatmap)
# Add legend title manually
grid::grid.text("Fold enrichment", x = 0.935, y = 0.97, rot = 0, gp = grid::gpar(fontsize = 12))
invisible(dev.off())
